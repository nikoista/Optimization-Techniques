...
%% METHODOS LEVENBERG_MARQUADRT ME XRHSH PARAGWGOU
...
%% LOOP LEVENBERG_MARQUADRT METHOD
%% GIA GAMMAk = STATHERO
...
while(k<1000)
    ...
    k
    fprintf(" LEVENBERG_MARQUADRT METHOD Gammak  IS A CONSTANT \n");
    fprintf("Gammak = %d \n",gammak);
    ...
    % LOOP GIA NA VROUME ENA mk POU KANEI TON HESSIAN THS F > 0
    mk=0.1;
    while(1)
        A = gradient2Fk + mk*I;
        if(det(A) > 0 &&  det(A(1,1)) > 0)
            break;
        end
        mk=mk+0.05;
    end
...
%% INITIALIZE SHMEIO EKKINHSHS (x0,y0)    =   (1,1) , (-1,-1) , (0,0)

x=x0;
y=y0;
G=[x0,y0];

%% EPOMENOS AKAIRIOS mk

k=1;
while(k<1000)
    
    k
    fprintf(" LEVENBERG_MARQUADRT METHOD Gamma IS MINIMIZED BY THE FUNCTION f(xk+g*dk) \n");
...
while(k<1000)
    ...
    k
    fprintf(" LEVENBERG_MARQUADRT METHOD Gamma IS SELECTED  BY THE  ARMIJO LAW fk+1< = fk + a*(b^mk)*s*dkGrad(fk) \n");
